...
import { useRecordOnsitePayment } from '../../features/checkin/hooks'
...
import type { OnsitePaymentResult } from '../../lib/api/checkin'
...
export default function CheckinOnsitePaymentPage() {
  ...
  const [paidReceipt, setPaidReceipt] = useState<OnsitePaymentResult | null>(null)

  const pendingReservations = useMemo(
    () => (reservations.data ?? []).filter((item) => item.status === 'PENDING'),
    [reservations.data],
  )
  ...
  function handleConfirmPayment() {
    if (!selected || recordPayment.isPending) return
    recordPayment.mutate(
      { reservationId: selected.id, amount: selected.amount },
      {
        onSuccess: (result) => setPaidReceipt(result),
      },
    )
  }
      ...
      {selected && (
        <div className="border border-line bg-white">
          <div className="flex items-start justify-between gap-3 border-b border-line p-5">
            ...
            <button
              type="button"
              onClick={() => { setSelected(null); setPaidReceipt(null) }}
              aria-label="닫기"
          ...
          <div className="border-t border-line p-5">
            ...
            {!paidReceipt ? (
              <button
                ...
                disabled={selected.status !== 'PENDING' || recordPayment.isPending}
              ...
              <div className="mt-4 border border-success bg-success/10 p-4">
                ...
                <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5 text-xs">
                  ...
                  <dd className="text-ink">{formatCurrency(paidReceipt.amount)}</dd>
            ...
            {recordPayment.isError && (
              <p className="mt-3 text-xs font-medium text-danger">결제 기록 중 오류가 발생했습니다. 다시 시도해주세요.</p>
            )}
          </div>
        </div>
      )}
