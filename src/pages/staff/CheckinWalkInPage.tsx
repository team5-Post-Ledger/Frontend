...
export default function CheckinWalkInPage() {
  ...
  const previewAmount =
    selectedTicketType && Number.isInteger(groupSizeNumber) && groupSizeNumber > 0
      ? selectedTicketType.price * groupSizeNumber
      : 0
  ...
  function handleConfirmPayment() {
    ...
    recordPayment.mutate(
      { reservationId: walkInResult.reservationId, amount: paymentAmount },
      {
        onSuccess: (result) => {
        ...
        <form onSubmit={handleInfoSubmit} className="border border-line bg-white p-5">
            ...
            <Field label="티켓 타입" id="walkin-ticket-type" required error={errors.ticketTypeId}>
              <QueryState isLoading={ticketTypes.isLoading} isError={ticketTypes.isError} height={44}>
                <select
                  ...
                  onChange={(event) =>
                    setForm((prev) => ({ ...prev, ticketTypeId: event.target.value ? Number(event.target.value) : null }))
                  }
                  className={fieldControlClass}
                >
          ...
          {selectedTicketType && (
            <p className="mt-3 text-xs text-muted">
              예상 결제 금액: <b className="text-ink">{formatCurrency(previewAmount)}</b> ({form.groupSize}명 ×{' '}
