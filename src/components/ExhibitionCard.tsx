...
export function ExhibitionCard({ exhibition }: { exhibition: Exhibition }) {
  ...
  return (
    <Link
      to={`/exhibitions/${exhibition.id}`}
      className="group relative block h-48 overflow-hidden border border-line bg-white transition-colors active:brightness-95 [@media(hover:hover)]:hover:border-primary [@media(hover:none)]:active:border-primary"
    >
      <div
        className="absolute inset-0 bg-cover bg-center opacity-15 transition-opacity duration-300 [@media(hover:hover)]:group-hover:opacity-100 [@media(hover:hover)]:group-focus-visible:opacity-100 [@media(hover:none)]:group-active:opacity-100"
